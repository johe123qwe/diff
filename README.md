...
+ 新增浅色 / 深色主题切换按钮，默认浅色。
+ 输入框下方新增「忽略空行」复选框，勾选后会先去掉两边的空行（只含空格、
  制表符等空白的行也算）再比较，适合一边分段留空行、另一边没留的情况。
  勾选状态会保留，输入框里始终保留原文，取消勾选即可回到原来的比较结果。
+ 新增「合并为一列」复选框，可把左右两列合并成一列连续文本，
  删除的内容标红并加删除线，新增的内容标绿；差异跳转、`n` / `p` 快捷键、
  「只显示有差异的行」、「黑白」打印配色在这个视图里同样可用。
+ 新增「忽略大小写」复选框。由于比较时每个字 / 词恰好是 `diff` 的一行，
  这个选项直接给 `diff` 加 `-i` 即可。同时把切分规则里的 `[a-z]+` 改成
  `[a-zA-Z]+`，否则 `The` 会被切成 `T` 和 `he`，大写开头的单词忽略大小写后
  仍然对不上（顺带也让英文的单词级比较更符合原本的设计意图）。
+ 新增「忽略行首尾空白」复选框，勾选后会先去掉每行行首、行末的空白
  （空格、制表符、全角空格，以及 CRLF 的 CR）再比较，适合缩进不一致的情况。
  和「忽略空行」一样，输入框里始终保留原文。
+ 输入框下方新增文件选择按钮，也可以直接把 `.txt` 等文本文件拖到输入框里。
  读取完全在浏览器端完成（`FileReader`，按 UTF-8 解码），不会额外上传文件，
  单个文件上限 10MB。选择框用 `accept` 过滤成常见文本格式（拖拽不受限制）；
  读入时会去掉 UTF-8 BOM，并检查内容里有没有 NUL 或大量 U+FFFD 替换字符——
  拖进 GBK 编码的文件或 `.docx` 这类二进制文件时会先提示确认，
  而不是默默塞一屏乱码进去。
+ 「比较」旁边新增「交换左右」和「清空」按钮（清空前会确认），
  在输入框里按 Ctrl / ⌘ + Enter 也可以直接提交比较。
+ 比较结果的两列各自加上行号，并可用「显示行号」复选框关掉（方便复制、打印）。
  行号按各自文本的真实行数来数：两列对齐时插入的空白行不占行号。
  列宽按最大行号的位数自动收缩，只有几行时不会留下一条空荡荡的白边。
+ 「忽略空行」「忽略大小写」「忽略行首尾空白」「只显示有差异的行」
  「合并为一列」「显示行号」和配色的选择都会记在浏览器本地（`localStorage`），
  下次打开自动恢复。输入类的选项只在首页恢复，结果页仍以本次比较实际使用的
  设置为准，避免勾选状态和结果对不上。
+ 大文本优化：把逐字切分改成一次性匹配（原来的写法耗时与文本长度的平方成正比），
  文本较大时自动不再使用 `diff -d`，并把跨行的高亮改为逐行闭合
  （原来的正则在大段差异上会触发 Perl 的递归上限）。
  同时放宽了请求体积上限，并为单侧文本加了 100 万字 / 词的上限，超出时给出提示。
  优化后处理时间和内存占用都与文本长度成正比：100 万字 / 侧，常见的
  「改过一版」场景约 3 秒 / 350MB，最坏情况（两边内容完全不同）约 19 秒 / 1.1GB。

  上限集中在各脚本开头，可按服务器内存自行调整，提示文字会跟着变：

  + `$maxtoken`：单侧最大字 / 词数，主开关。
  + `$maxchar`：单侧最大字符数，默认为 `$maxtoken` 的 4 倍。这是切分**之前**的
    足切，用来挡住超大请求——若等切分完再判断，光是拒绝一个 11MB 的请求就会
    占用 1.2GB 内存。
  + `$maxrequest`：请求体上限。UTF-8 中文经 URL 编码后 1 字 = 9 字节，
    100 万字两侧约 17MB，因此默认 30MB。
    注意 nginx 的 `client_max_body_size` 默认只有 1MB，需要一并调大。

除「忽略空行」「忽略大小写」「忽略行首尾空白」和大文本优化之外，其余改动
都只涉及页面展示；前三项也只是在送进 `diff` 之前对文本做一次归一化，
差异本身仍然由原项目的 `diff` 流程算出，比较过程也依然不把文档写入磁盘。
